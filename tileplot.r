...
tile.plot <- function(naaqs,geo,state,out) {
  ...
  cnames <- colnames(eval(parse(text=paste("sitedv",std,sep="."))))
  dv.year <- as.numeric(substr(cnames[grep("dv",cnames)],9,12))
  all.years <- c((dv.year-12):dv.year)
  ...
  if (length(curr) > 0) { temp <- ldply(curr,function(x) rbind(x$data)) }
  if (length(hist) > 0) { temp <- rbind(temp,ldply(hist,function(x) rbind(x$data))) }
  ...
  for (year in all.years) {
    if (length(grep(year,colnames(temp))) == 0) { 
      temp[,paste(year,"dmax",sep="_")] <- rep(NA,nrow(temp)) 
    }
  }
  temp <- temp[,c("day",paste(all.years,"dmax",sep="_"))]
  ...
  plot(x=NULL,y=NULL,type='n',axes=FALSE,bg="gray75",xlim=c(0,366),xaxs="i",xlab="",
    ylim=c(dv.year+0.5,all.years[1]-0.5),yaxs="i",ylab="",
    main=paste(title1,title2,sep=" - "))
  polygon(x=par("usr")[c(1,2,2,1)],y=par("usr")[c(3,3,4,4)],col="#C0C0C0")
  image(x=c(0:366),y=all.years,z=as.matrix(tileplot.vals[,-1]),breaks=aqi.breaks,
  ...
  y.ticks <- c(all.years-0.5,dv.year+0.5)
  ...
  mtext(all.years,side=2,line=0.25,at=all.years,las=2,cex=1.5)
  box()
  rect(xleft=c(0,0.125,0.265,0.545,0.695,0.89)*366,ybottom=dv.year+1.2,
    xright=c(0.025,0.15,0.29,0.57,0.72,0.915)*366,ytop=dv.year+1.6,border="#000000",
  ...
  text(labels=c("Good","Moderate","Unhealthy Sens. Groups","Unhealthy","Very Unhealthy",
    "No Data"),x=c(0.025,0.15,0.29,0.57,0.72,0.915)*366,y=dv.year+1.4,cex=1.5,pos=4)
  text(labels=paste(break.min,"-",break.max,"ppb"),
    x=c(0,0.125,0.265,0.545,0.695)*366,y=dv.year+2,cex=1.5,pos=4)
}
