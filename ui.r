...
## Function to retrieve most recent update date
update.date <- function() {
  files <- list.files("data")
  dv.file <- rev(files[grep("sitedv",files)])[1]
  file.date <- gsub("sitedv_","",gsub(".Rdata","",dv.file))
  update.date <- as.POSIXlt(file.date,format="%Y%m%d")
  return(format(update.date,"%B %d, %Y"))
}
...
shinyUI(fluidPage(title="Ozone Watch",
  ...
  absolutePanel(id="heading-panel",h1(paste("Ozone Watch: Updated",update.date())),
  ...
  absolutePanel(id="control-panel",
    ...
    selectInput(inputId="app.select",label="Select an Application:",
      choices=c("Design Value Tables","Design Value Maps",
    ...
    top="80px",left="0px",height="720px",width="320px"),
  ...
  absolutePanel(id="display-panel",
    ...
    conditionalPanel(id="dvtables-intro",
      ...
      p("This application displays tables of preliminary design value information for the
         current 3-year period based on the data available to-date. The buttons on the left
         allow users to switch between area-level summary information based on the monitor
         with the highest design value in each geographic area, and more detailed site-level
         information."),
      p("The remaining menus allow users to choose which ozone NAAQS the design values will
         ...
         statistics, and finally to subset the results to areas or sites within a particular
         EPA Region or State. With the site-level option, there is an additional menu which
         allows the user to further subset the results to a single geographic area such as a
         county or CBSA."),
    ...
    conditionalPanel(id="areamaps-intro",
      ...
      p("This application generates maps of the preliminary design values and annual 4th
         highest daily maximum values for the current 3-year period. The buttons on the left
         allow the user to switch between maps showing area-level boundaries filled with
         colors based on the highest monitored value in that area, and maps showing values
         for individual monitoring sites, which are represented by colored dots."),
      p("The remaining menus allow users to choose which ozone NAAQS the design values will
         be compared with, to choose which type of geographic area boundaries will be shown
         on the map, and finally to show localized maps of individual EPA Regions or States.
         With the site-level option, there is an additional menu option which can generate
  ...
  absolutePanel(id="text-panel",
    ...
    top="800px",left="320px",height="160px",width="960px")
))
