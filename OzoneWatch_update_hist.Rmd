...
get.monitors <- function(par,yr1,yr2,all=FALSE) {
  ...
  ORDER BY 1",sep=""))
  ## Fix issue where lat/lon coordinates are returned as character fields
  t$latitude <- as.numeric(t$latitude); t$longitude <- as.numeric(t$longitude);
  ...
  if (par == 88101) {
    sjv <- grep("San Joaquin",t$naa_name_1997)
    t$naa_name_1997[sjv] <- "San Joaquin Valley, CA"
  }
  if (all) { monitors <- t }
  if (!all) {
    ...
    monitors$season_begin_date[which(monitors$network == "NCORE")] <- "01-01"
    monitors$season_end_date[which(monitors$network == "NCORE")] <- "12-31"
  }
  return(monitors)
}
```
...
get.raw.data <- function(par,year) {
  ...
  ## Fix issue with duplicate exceptional event concurrence indicators
  if (any(duplicated(aqs.data[,c("id","dt")]))) {
    dup.ind <- which(duplicated(aqs.data[,c("id","dt")]))
    dup.ind <- c((dup.ind-1),dup.ind)
    dup.ind <- dup.ind[order(dup.ind)]
    vals.rm <- dup.ind[which(aqs.data$concur[dup.ind] == "N")]
    aqs.data <- aqs.data[-c(vals.rm),]
  }
...
## Function to calculate the cumulative number of exceedance days per year
cum.exc <- function(x,lvl) { return(cumsum(replace(floor(x),which(is.na(x)),0) > lvl)) }
...
for (naaqs in c("2015","2008","1997")) {
  ...
  for (i in 1:length(site.vals)) {
    ...
    site.vals[[i]]$exc <- unlist(tapply(site.vals[[i]]$dmax,list(site.vals[[i]]$year),cum.exc,lvl=level))
