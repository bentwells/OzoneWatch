...
dv.trends <- function(naaqs,geo,state,out) {
  ...
  if (length(hist) > 0) { 
    ...
    t2 <- ldply(hist,function(x) rbind(subset(x$daily_data,day == substr(curr.date,6,10))))
    max.curr <- ddply(t2,"year",summarize,max4_curr=max.na(max4))
  ...
  if (length(curr) > 0) { 
    ...
    trend.curr <- data.frame(year=curr.year,dv=ifelse(this.year,NA,max.na(temp$dv)),
      max4=ifelse(this.year,NA,max.na(temp$dv)),dv_site=dv.site,max4_site=max4.site,
      max4_curr=max.na(temp$max4))
  }
  trend.data <- rbind(trend.hist,trend.curr)
  if (nrow(trend.data) < 3) { trend.data$dv_curr <- NA }
  if (nrow(trend.data) >= 3) {
    trend.data$dv_curr <- c(NA,NA,floor((trend.data$max4[1:(nrow(trend.data)-2)] + 
      trend.data$max4[2:(nrow(trend.data)-1)] + trend.data$max4_curr[3:nrow(trend.data)])/3))
  }
