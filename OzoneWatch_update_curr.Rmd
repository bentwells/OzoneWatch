...
aqs <- subset(get.aqs.data(paste("SELECT DISTINCT
  ...
  if (any(duplicated(aqs[,c("id","dt")]))) {
    dup.ind <- which(duplicated(aqs[,c("id","dt")]))
    dup.ind <- c((dup.ind-1),dup.ind)
    dup.ind <- dup.ind[order(dup.ind)]
    vals.rm <- dup.ind[which(aqs$concur[dup.ind] == "N")]
    aqs <- aqs[-c(vals.rm),]
  }
...
for (a in c("U","P","I")) {
  ...
  for (i in 1:length(site.vals)) {
    ...
    site.vals[[i]]$exc <- cum.exc(site.vals[[i]]$dmax,level)
