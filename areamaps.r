...
area.maps <- function(naaqs,type,geo,region,state,out,value) {
  ...
  if (dv.type == "area") {
    ...
    assign("map.shape",SpatialPolygonsDataFrame(SpatialPolygons(
      mapply(function(poly,id) { Polygons(lapply(split(poly[,c("lon","lat")],poly$id),Polygon),ID=id) },
      poly=split(area.shape[,c("id","lon","lat")],area.shape$fips),id=area.info$fips)),
      data.frame(vals[order(area.info$fips),],row.names=area.info$fips)),envir=sys.frame(0))
  }
